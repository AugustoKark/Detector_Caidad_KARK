...
import com.kark.falldetector.BuildConfig
import com.kark.falldetector.alerts.FallAlertActivity
...
import com.kark.falldetector.utils.FallDetectionSettings
...
import com.kark.falldetector.alerts.FallCountdownService
...
class Detector private constructor() : SensorEventListener {
    ...
    private fun process() {
        ...
        if (0 == timeoutImpact) {
            ...
            log(android.util.Log.DEBUG, "Fall check - Orient: $orientationChanged, Horiz: $isHorizontal, Vert: $isVertical, Type: $fallType, Peak: $peakAcceleration, PhoneLook: $isPhoneLook, InPocket: ${isInPocket()}, Proximity: ${proximityValue}cm")
            ...
            if (hasStrongEvidence) {
                ...
                if (isInPocket()) {
                    ...
                    if (context != null && FallDetectionSettings.isFallDetectionEnabled(context)) {
                        ...
                        Guardian.say(context, android.util.Log.WARN, TAG, "Detected a $detectionType IN POCKET - Reason: $fallReason, Peak: $peakAcceleration, Proximity: ${proximityValue}cm")

                        // MOSTRAR ALERTA SOLO SI ESTÁ EN EL BOLSILLO
                        showFallAlert(context)

                        if (esFalsaCaida) {
                            log(android.util.Log.INFO, "FALSE_POSITIVE detected in pocket, logged, and alert shown for confirmation")
                        } else {
                            log(android.util.Log.INFO, "TRUE_FALL detected in pocket and alert shown")
                        }
                    } else {
                        context?.let {
                            Guardian.say(it, android.util.Log.INFO, TAG, "Fall detected in pocket but detection is disabled in settings")
                        }
                    }
                } else {
                    // Caída detectada pero NO está en el bolsillo - NO mostrar alerta
                    log(android.util.Log.INFO, "Fall detected but device NOT in pocket - Proximity: ${proximityValue}cm - Skipping alert")
                }
            } else {
